...
static func setup_collision_rectangle(tilemap: TileMapLayer, top_left: Vector2i, bottom_right: Vector2i, tile_source_id: int = 0, layer: int = 0):
	"""
	Sets up collision tiles in a rectangular area
	tilemap: The TileMapLayer node
	top_left: Top-left corner in tile coordinates 
	bottom_right: Bottom-right corner in tile coordinates
	tile_source_id: Which tile to use (0 = red collision tile by default)
...
static func setup_collision_line(tilemap: TileMapLayer, start: Vector2i, end: Vector2i, tile_source_id: int = 0, layer: int = 0):
	"""
	...
	for point in points:
		tilemap.set_cell(point, tile_source_id, Vector2i(0, 0))
...
static func clear_collision_rectangle(tilemap: TileMapLayer, top_left: Vector2i, bottom_right: Vector2i, layer: int = 0):
	"""
...
static func setup_room_walls(tilemap: TileMapLayer, top_left: Vector2i, bottom_right: Vector2i, tile_source_id: int = 0, layer: int = 0):
	"""
...
static func setup_example_town_collision(tilemap: TileMapLayer):
	"""
	Example of how to efficiently set up collision for a whole town
	"""
	
	# Map boundaries (adjust these to your map size)
	setup_room_walls(tilemap, Vector2i(-20, -15), Vector2i(20, 15), 0, 0)
	
	# Buildings
	setup_collision_rectangle(tilemap, Vector2i(-10, -10), Vector2i(-5, -5), 0, 0) # Building 1
	setup_collision_rectangle(tilemap, Vector2i(5, -8), Vector2i(10, -3), 0, 0) # Building 2
	setup_collision_rectangle(tilemap, Vector2i(-8, 5), Vector2i(-3, 10), 0, 0) # Building 3
	
	# Fences or barriers
	setup_collision_line(tilemap, Vector2i(-15, 0), Vector2i(-12, 0), 0, 0) # Fence
	setup_collision_line(tilemap, Vector2i(12, -2), Vector2i(15, 2), 0, 0) # Barrier
